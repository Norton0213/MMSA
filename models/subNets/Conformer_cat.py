...
class Conformer(torch.nn.Module):
    def __init__(self, n_mels=80, num_blocks=6, output_size=256, embedding_dim=192, input_layer="conv2d2",
        ...
        self.conformer = ConformerEncoder(input_size=n_mels, num_blocks=num_blocks,
                output_size=output_size, input_layer=input_layer, pos_enc_layer_type=pos_enc_layer_type)#input_layer是进conformer前的卷积下采样层


    def forward(self, feat):
        #feat = feat.squeeze(1).permute(0, 2, 1)
        lens = torch.ones(feat.shape[0]).to(feat.device)
        lens = torch.round(lens*feat.shape[1]).int()#round舍入到最近的整数
        x, masks, mfa = self.conformer(feat, lens)
        """"    
        x = x.permute(0, 2, 1)
        x = self.pooling(x)
        x = self.bn(x)
