...
class MISA(nn.Module):
    def __init__(self, config):
        ...
        self.input_sizes = input_sizes = [self.text_size, self.visual_size, self.acoustic_size]#[768,709,33]
        self.hidden_sizes = hidden_sizes = [int(self.text_size), int(self.visual_size), int(self.acoustic_size)]#[768,709,33]
        self.output_size = output_size = config.num_classes if config.train_mode == "classification" else 1#1
        self.dropout_rate = dropout_rate = config.dropout
        self.activation = nn.ReLU()
        self.tanh = nn.Tanh()
        
        if config.use_bert:#文本子网络，用bert
            # text subnets
        ...
        self.arnn1 = rnn(input_sizes[2], hidden_sizes[2], bidirectional=True)#(33,33)
        self.arnn2 = rnn(2*hidden_sizes[2], hidden_sizes[2], bidirectional=True)#(66,33)

        """
        # 定义Conformer块
        self.vConformer = conformer(n_mels=input_sizes[1], num_blocks=1, output_size=256, embedding_dim=192, input_layer="conv2d", pos_enc_layer_type="rel_pos")
        self.aConformer = conformer(n_mels=input_sizes[2], num_blocks=1, output_size=256, embedding_dim=192, input_layer="conv2d", pos_enc_layer_type="rel_pos")
        """

        ##########################################
        # mapping modalities to same sized space
        ##########################################
        if self.config.use_bert:#全连接改维度
        ...
        self.fusion.add_module('fusion_layer_1', nn.Linear(in_features=self.config.hidden_size*6, out_features=self.config.hidden_size*3))#(768,384)
        self.fusion.add_module('fusion_layer_1_dropout', nn.Dropout(dropout_rate))
        self.fusion.add_module('fusion_layer_1_activation', self.activation)
        self.fusion.add_module('fusion_layer_3', nn.Linear(in_features=self.config.hidden_size*3, out_features= output_size))#(384,1)

        self.tlayer_norm = nn.LayerNorm((hidden_sizes[0]*2,))#(768*2)
        self.vlayer_norm = nn.LayerNorm((hidden_sizes[1]*2,))#(709*2)
        self.alayer_norm = nn.LayerNorm((hidden_sizes[2]*2,))#(88*2)
    ...
    def extract_features(self, sequence, lengths, rnn1, rnn2, layer_norm):
        ...
        packed_sequence = pack_padded_sequence(sequence, lengths, batch_first=True, enforce_sorted=False)# 压紧


        if self.config.rnncell == "lstm":
            packed_h1, (final_h1, _) = rnn1(packed_sequence)#输出数据包括output, (h_t, c_t)，output.shape = [seq_length, batch_size, num_directions * hidden_size]，
        else:
            packed_h1, final_h1 = rnn1(packed_sequence)#h_t.shape = [num_directions * num_layers, batch, hidden_size]

        padded_h1, _ = pad_packed_sequence(packed_h1)#填充
        padded_h1 = padded_h1.permute(1, 0, 2)
        normed_h1 = layer_norm(padded_h1)
        packed_normed_h1 = pack_padded_sequence(normed_h1, lengths, batch_first=True, enforce_sorted=False)#再压紧
    ...
    def alignment(self, text, acoustic, visual):
        ...
        #acoustic = acoustic.expand(-1, 40, -1)
        ...
        #Conformer特征提取模块
        # extract features from acoustic modality
        utterance_audio = self.aConformer(acoustic)
        utterance_audio = utterance_audio.squeeze(1)

        # extract features from visual modality
        utterance_video = self.vConformer(visual)
        utterance_video = utterance_video.squeeze(1)
        """
